...
#import pendulum
import uvicorn
from T13_subt4_rag_backend import *
...
async def get_pdf(url,name):
    return {"ret":await download_pdf(url,name)}

@app.get("/load_pdf/")
async def print_pdf(name):
    pdf_content = await load_pdf(name)
    text = extract_text_from_pdf(pdf_content)
    return {"ret": text}

@app.get("/preprocess_pdf/")
async def preprocess_pdf(name):
    
    pdf_content = await load_pdf(name)
    doc_splitted = await split_doc(pdf_content)
    return {"doc_splitted": str(doc_splitted)}

@app.get("/gen_context/")
async def gen_context(name):
...
if __name__ == "__main__":
    uvicorn.run("T13_subt4_main:app", reload=True)
