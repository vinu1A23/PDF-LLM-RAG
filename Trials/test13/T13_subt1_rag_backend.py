...
os.environ["cache_directory"] = str(os.path.join(os.getcwd(),"cache"))
cache_directory = os.getenv("cache_directory")

os.environ['HF_HOME'] = str(os.path.join(os.getcwd(),"cache"))
...
async def download_pdf(url,name):
    ...
    #app.logger.info(f"file_name is ,{file_name}")
...
async def load_pdf(name):
    ...
    if name is None or name == "":
        ...
        file_path = name
    #app.logger.info(f"file path given as  {file_path}")
...
async def split_doc(content):
    ...
    #app.logger.info(f"***** Logging the first doc split****   {document_splitted[0]}")
...
def load_embedding(
    ...
    #app.logger.info(" Printing result of query result "+str(query_result[:3]))
    return embeddings
...
def generate_context(db, query):
    ...
    #app.logger.info(f" *****, Q was , {question} ***answer is within doc split ** "+str(searchDocs[0].page_content))
    #app.logger.info(" len of searchDocs is "+ str(len(searchDocs)))
    context = ""
...
def answer_query(context, query, model_to_be_used, tokenizer_to_be_used):
    ...
    #app.logger.info(f"the response generated is {response}")
    #app.logger.info(f"The knowledge used was {context}")
    return response
